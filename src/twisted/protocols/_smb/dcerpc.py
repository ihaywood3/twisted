...
def referent():
    ...
    """
    return attr.ib(
        factory=_referent_factory, type=int, metadata={base.SMB_METADATA: "I"}
    )
...
def _unpackWchar(data, offset):
    ...
    assert (
        data[offset + (actual_count - 1) * 2 : offset + (actual_count * 2)] == b"\0\0"
    )
...
class DceRpcProcessor:
    ...
    def dataReceived(self, data):
        ...
        while True:
            if len(self.buffer) < base.calcsize(DceHeader):
                return
            header, offset = base.unpack(DceHeader, self.buffer, 0, base.OFFSET)
            if self._sanity_check(header):
                self.buffer = b""
                return
            if len(self.buffer) < header.frag_length:
                return
            self.fragmentReceived(header, self.buffer[offset : header.frag_length])
    ...
    def _sanity_check(self, header):
        ...
        if header.rpc_vers != RPC_VERSION:
            ...
            version = True
            log.error(
                "DCE/RPC major version {n}, must be {m}",
                n=header.rpc_vers,
                m=RPC_VERSION,
            )
        if header.rpc_vers_minor > RPC_VERSION_MINOR:
            ...
            version = True
            log.error(
                "DCE/RPC minor version {n}, must be {m} or lower",
        ...
        if insane and ptype_name == "bind":
            ...
            if version:
                nak = BindNak(
                    provider_reject_reason=BindNakFailure.PROTOCOL_VERSION_NOT_SUPPORTED.value,
                    n_versions=1,
                )
                vers = BindNakVersion(major=RPC_VERSION, minor=RPC_VERSION_MINOR)
                self.send(
                    "bind_nak", base.pack(nak), base.pack(vers), callid=header.callid
                )
            else:
                nak = BindNak(
                    provider_reject_reason=BindNakFailure.REASON_NOT_SPECIFIED.value,
                    n_versions=0,
                )
    ...
    def dcerpc_bind(self, header, sec_header, payload):
        ...
        for i in range(sec_header.n_contexts):
            ...
            if found_ts:
                ...
                replies.append(
                    ResultItem(
                        result=BindAckResult.ACCEPTANCE.value,
    ...
    def dcerpc_request(self, header, sec_header, payload):
        ...
        log.debug(
            "DCE/RPC request {pipe}/{opnum}", pipe=self.pipe, opnum=sec_header.opnum
        )
        try:
            ...
            self.send_fault(
                FaultStatus.unsupported_operation.value, p_cont_id, callid, no_exec=True
            )
            return
    ...
    def send_fault(self, status, p_cont_id, callid, no_exec=False):
        ...
        self.send("fault", base.pack(resp), b"", callid=callid, fault_noexec=no_exec)

    def send(self, ptype_name, sec_header, payload, callid, fault_noexec=False):
