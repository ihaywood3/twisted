...
def _unpackWchar(data, offset):
    max_count, offset2, actual_count = _3ints.unpack_from(data, offset)
    offset += _3ints.size
    s = data[offset + (offset2 * 2):offset +
                       (actual_count - 1) * 2]
...
class DceRpcProcessor:
    ...
    def _sanity_check(self, header):        
        ...
        if insane and ptype_name == 'bind':
            ...
            if version:
                ...
                self.send('bind_nak', base.pack(nak), base.pack(vers), callid=header.callid)
            else:
                nak = BindNak(
    ...
    def dcerpc_bind(self, header, sec_header, payload):
        ...
        for i in range(sec_header.n_contexts):
            ...
            log.debug("DCE/RPC bind presentation context abstract_uuid={u!r}.{v!r}", u=pc.abstract_uuid, v=pc.abstract_version)
            found_ts = False
            ts_version = 0
    ...
    def send(self, ptype_name, sec_header, payload, callid, fault_noexec=False):
        ...
        header = DceHeader(
            callid = callid,
            drep = bytes([
                DREP_INT_LITTLEENDIAN,
                FloatFormat.IEEE.value,
                0, 0]),
